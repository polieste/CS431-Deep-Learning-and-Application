...
class cnn():
    ...
    def build(self, in_dim = (1, 1), out_dim = 1):
        ...
        hidden = layers.Conv2D(8, (3, 3), activation=None) (input)
        hidden = layers.MaxPooling2D((2, 2)) (hidden)

        hidden = layers.Conv2D(32, (3, 3), activation=None) (hidden)
        hidden = layers.MaxPooling2D((2, 2)) (hidden)

        hidden = layers.Conv2D(10, (4, 4), activation=None) (hidden)
        hidden = layers.MaxPooling2D((2, 2)) (hidden)

        hidden = layers.Flatten() (hidden)
        hidden = layers.Dense(64,activation=None) (hidden)
        output = layers.Dense(out_dim, activation=None) (hidden)
...
model.save("CNN WITHOUT ACTIVATION")
model.load("CNN WITHOUT ACTIVATION")


'''# Step 4: Visualize Result
PATH = "cnn without activation"
# MINIST Data
visualize_data(train_images, train_labels, class_names)
plt.savefig('data_activation.png')
plt.show()

# Training
visualize_training(hist)
plt.savefig(PATH + 'training_activation.png')
plt.show()

# Result
predictions = model.predict(x_test)
visualize_result(test_labels, predictions, 'CONFUSION MATRIX', class_names)
plt.savefig(PATH + 'confusion_matrix_activation.png')
plt.show()

# Example
visualize_example(predictions, test_labels, test_images, class_names)
plt.savefig(PATH + "result_activation.png")
plt.show()

# save statistic
model.save_statistic('statistics_activation.csv')'''
