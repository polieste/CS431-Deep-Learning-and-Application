...
class cnn():
    ...
    def build(self, in_dim = (1, 1), out_dim = 1):
        ...
        hidden = layers.Conv2D(10, (4, 4), activation="relu") (hidden)

        hidden = layers.Flatten() (hidden)
        hidden = layers.Dense(64,activation=None) (hidden)
        output = layers.Dense(out_dim, activation="softmax") (hidden)
...
model.save("CNN WITHOUT pooling")
model.load("CNN WITHOUT pooling")


'''# Step 4: Visualize Result
PATH = "cnn without pooling"
# MINIST Data
visualize_data(train_images, train_labels, class_names)
plt.savefig('data_pooling.png')
plt.show()

# Training
visualize_training(hist)
plt.savefig(PATH + 'training_pooling.png')
plt.show()

# Result
predictions = model.predict(x_test)
visualize_result(test_labels, predictions, 'CONFUSION MATRIX', class_names)
plt.savefig(PATH + 'confusion_matrix_pooling.png')
plt.show()

# Example
visualize_example(predictions, test_labels, test_images, class_names)
plt.savefig(PATH + "result_pooling.png")
plt.show()

# save statistic
model.save_statistic(PATH + 'statistics_pooling.csv')'''
